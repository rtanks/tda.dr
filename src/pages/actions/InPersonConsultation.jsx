...
export default function InPersonConsultation() {
    ...
    return (
        <div className="w-full h-full bg-white">
            <TestHeader title={"مشاوره حضوری"} onClick={() => navigate("/account/need-for-action")}/>
            <div className="w-full vazir-medium h-max">
                ...
                <div className="w-full h-max bg-[#f5f5f5] flex flex-col gap-2">
                    <AddTime title={"صبح"} times={times.filter(time => time.time == "morning")}/>
                    <AddTime title={"ظهر"} times={times.filter(time => time.time == "noon")}/>
                    <AddTime title={"عصر"} times={times.filter(time => time.time == "afternoon")}/>
                </div>
            </div>
        </div>
    )
