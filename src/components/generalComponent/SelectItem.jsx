...
export default function SelectItem({selectItem,onClick, title, status}) {
    ...
    return(
        <div onClick={() => onClick(title)}
            ...
            {(status == "complete" ? true : false) ? <LuCircleCheckBig size={15}/> : <LuCircle size={15} className="text-[#676767]"/>}
            {title}
        </div>
    )
}